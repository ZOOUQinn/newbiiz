...
class EdiProductMalabsCADRecord(models.Model):
    ...
    def _product_values(self, record_vals):
        ...
        product_vals.update({
            ...
            'list_price': record_vals.get('price', 0)*self.doc_id.currency_rate,
            'standard_price': record_vals.get('cost', 0)*self.doc_id.currency_rate,
        })
...
class EdiProductMalabsCADDocument(models.AbstractModel):
    ...
    def _record_values(self, data):
        ...
        for val in values:
            ...
            d['image'] = base64.b64encode(requests.get(d.get('image')).content).decode("utf-8")
