...
class MalabsBinder(AbstractComponent):

    """ Generic Binder for MalabsCommerce """
    _name = 'malabs.binder'
    _inherit = ['base.binder']


class MalabsModelBinder(Component):

    """
    Bindings are done directly on the binding model.malabs.product.category

    Binding models are models called ``malabs.{normal_model}``,
    like ``malabs.res.partner`` or ``malabs.product.product``.
    They are ``_inherits`` of the normal models and contains
    the Malabs ID, the ID of the Malabs Backend and the additional
    fields belonging to the Malabs instance.
    """
    _name = 'malabs.model.binder'
    _inherit = ['malabs.binder']
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                malabs_id = binding.malabs_id
                if malabs_id == self.component(usage='backend.adapter', model_name=self.model._name).read(malabs_id).get('id'):
                    return binding
                else:
                    binding.unlink()
            return None
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Malabs! Prevent False values
        ...
        # avoid to trigger the export when we modify the `malabs_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``malabs.product.product`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``malabs.product.product``,
